...
// TestGUISubsystem builds the mtui-hook binary with -H windowsgui and
...
// silently reintroducing the console-window flash on every hook event.
func TestGUISubsystem(t *testing.T) {
	...
	outExe := filepath.Join(t.TempDir(), "mtui-hook-test.exe")
	...
	t.Logf("mtui-hook.exe PE Subsystem = %d", subsystem)
	if subsystem != 2 {
