<script lang="ts">
  ...
  $: if (visible) {
    ...
    selectedLang = ($config.language || 'de') as Language;
    claudeEnabled = $config.claude_enabled !== false;
    claudeCommand = $config.claude_command || '';
    codexEnabled = $config.codex_enabled || false;
    codexCommand = $config.codex_command || '';
    geminiEnabled = $config.gemini_enabled || false;
    geminiCommand = $config.gemini_command || '';
