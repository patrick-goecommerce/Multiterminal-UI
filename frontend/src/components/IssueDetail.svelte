...
<div class="detail">
  <div class="detail-header">
    ...
    <button
      ...
      title={$t('issueDetail.toggleState')}
    >
    ...
    {/if}
    <button class="edit-btn" on:click={() => dispatch('editIssue', issue)} title={$t('issueDetail.edit')}>
      <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
  ...
  {#if issue.comments && issue.comments.length > 0}
    <div class="comments-header">{$t('issueDetail.comments', { count: issue.comments.length })}</div>
  ...
  <div class="comment-form">
    ...
    <button class="send-btn" on:click={submitComment} disabled={!commentText.trim() || submitting}>
      {submitting ? $t('issueDetail.sending') : $t('issueDetail.send')}
    </button>
  </div>
</div>
