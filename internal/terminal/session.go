...
type Session struct {
	...
	// job holds the current generation's process tree (see procs.Job). It is
	// released when the generation ends on purpose; if this process dies
	// instead, the kernel ends every process in it.
	job *procs.Job
